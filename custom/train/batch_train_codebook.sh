...
# Config
BATCH_CONFIG="custom/configs/batch_codebook/batch_codebook.yaml"
PYTHON_RUNNER="python custom/train/batch_train_codebook.py"
...
if ! command -v tmux >/dev/null 2>&1; then
    echo "tmux not found. Please install tmux, or run train_frame_codebook.py manually."
    exit 1
fi
...
session_name = tmux_cfg.get("session_name", "temp_codebook_train")
...
num_gpus=${#gpu_list[@]}
echo "Available GPUs: ${gpu_list[*]} ($num_gpus GPUs)"
...
for c in${assigned}; do
  ...
  tmux pipe-pane -t "$SESSION_NAME:$win_name" "cat > custom/logs/batch_\${c}.log"
  $PYTHON_RUNNER --config "$BATCH_CONFIG" --case "\$c" --device "cuda:$gpu"
  echo "[GPU $gpu] done: \$c"
done
