...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train official ID-cond EMG->pose and evaluate with official metrics.")
    ...
    parser.add_argument("--train_filelist", type=str, default="musclesinaction/ablation/generalization_ID_cond_exercises_nn/trainSubject0.txt")
    parser.add_argument("--val_for_selection", type=str, default="musclesinaction/ablation/generalization_ID_cond_exercises_nn/valRunning_Subject0.txt")
    parser.add_argument("--eval_dir", type=str, default="musclesinaction/ablation/generalization_ID_cond_exercises_nn")
    parser.add_argument("--name", type=str, default="official_reproduction_cond_emgtopose_threed", help="Experiment name (checkpoint subdir name).")
    ...
    _run(
        ...
        cwd=str(mia_root),
        env=env,
        log_file=logs_root / "train_cond_emgtopose.log",
    ...
    for fp in val_lists:
        ...
        eval_cmd = [
            sys.executable,
            "musclesinaction/inference_scripts/inference_id_transf_cond_exercises_emgtopose.py",
            "--name",
            str(args.name),
            "--std",
            "False",
