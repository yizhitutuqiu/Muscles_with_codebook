...
scores.sort(key=lambda x: x[0], reverse=False) # sort ascending
for i in range(min(5, len(scores))):
    print(f"Worst Rank {i+1}: Exercise {scores[i][3]} Sample {scores[i][4]} | Score(Diff): {scores[i][0]:.4f} | Official: {scores[i][1]:.4f} | Our: {scores[i][2]:.4f}")
