...
old_func = """def _infer_our_emg2pose(model, stage1, emg_8_t, condval, device, joints3d_t_25_3) -> np.ndarray:
    import torch
    inputs_np = emg_8_t.T
...
new_func = """def _infer_our_emg2pose(model, stage1, emg_8_t, condval, device, joints3d_t_25_3) -> np.ndarray:
    ...
    # CRITICAL FIX: The official Mia_style_eval uses root_index=0 for root_centering!
    # "data": {"step": 30, "std": False, "percent": 1.0, "joints3d_root_center": True}
    # and the default root_index in _eval_stage2_on_filelist is 0.
    # So our model's output is centered around Joint 0.
    joints3d_t_25_3 = joints3d_t_25_3[:, :25, :]
    root_0 = joints3d_t_25_3[:, 0:1, :]
    return pred_np + root_0"""
