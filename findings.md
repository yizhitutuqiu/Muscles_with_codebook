...
- **H8 专家扩容与盲路由的确立**：将 MoE 专家数增加至 8 个后，无条件 RMSE 进一步下降至 **10.16**。尝试使用 Codebook 进行显式的 MoE 路由引导（H8 与 H9）均以失败告终，这揭示了宏观动作语义与微观 Token 路由之间存在“粒度错位”。因此，确立了基于纯连续特征的**盲路由（Blind Routing）机制**作为核心。
- **高质量语义 Codebook 的作用 (H9-v2)**：通过为 Stage 1 VQ-VAE 增加“共享双头分类辅助约束 (Auxiliary Cross-Entropy)”，我们在不破坏 OOD 泛化能力的前提下，成功赋予了 Codebook 明确的运动学语义边界。使用该 Codebook 结合 8 专家盲路由，最终将平均 RMSE 压低至 **10.14**，证明了“高质量的离散结构化先验”才是下游任务表现的基石。
- **实验基础设施的稳固 (Bug Fixes)**：修复了由于 `musclesinaction.dataloader.data` 导入引发的全局随机种子强制重置（导致多卡模型初始化完全相同）的隐蔽 Bug，并修复了外部 YAML 配置未能正确覆盖 Checkpoint 内部参数的问题，确保了后续消融实验的绝对严谨性。
...
### 阶段 7：H8 专家扩容与 Guided MoE 的证伪
- **专家扩容的收益**：在 H7 双分支并行的基础上，将 MoE 的专家数从 4 个扩容至 8 个（`exp_h8_baseline_8exp`），无条件 RMSE 成功从 10.30 进一步下降至 **10.16**，说明该架构对参数量的扩容仍有明显的收益。
- **Codebook-Guided MoE 验证失败**：
  - **实验设计**：我们尝试用离散 Codebook 特征通过 Semantic Bias（策略 2）和 Cross-Attention（策略 3）来硬引导或软引导 MoE 的路由分配。
  - **结果**：这两种方法均导致性能退化（Bias 降至 10.50，Cross-Attn 降至 10.20）。
  - **分析与洞察**：离散词汇代表的是一个动作片段的“宏观语义”，而肌电信号在帧级别（Token-level）充满了复杂的高频抖动。强制让宏观语义去指挥微观的特征路由，反而打破了网络自发寻找局部最优专家的能力，造成了“大类标签裹挟局部细节”的粒度错位现象。
- **决策**：废弃离散引导路由策略，**锁定并保留基于纯连续特征的盲路由（Blind Routing）机制**。目前的最优解确立为：**8 专家 + 双向对称 DCSA + 双分支并行 DSTFormer (RMSE: 10.16)**。
